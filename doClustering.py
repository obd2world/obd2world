...
################################################################################
    def Run(self, datafile, labelfile, evidence=0, print_data=False):
      ...
      if (self.serialize_kmeans_tsne == False):
          ...
          if evidence > 0:
              self.sc.append(plt.scatter(self.X_tsne[0][:evidence,0], self.X_tsne[0][:evidence,1], s=150, c="#FFFF00"))
...
myView=VecView(encoding=ENCODE, num_kmeans_clusters=10, serialize_kmeans_tsne=False)
...
myView.Run(BASEPATH + TASK + '/accertamentiset.npy', BASEPATH + TASK + '/labels.txt', 0, True)
